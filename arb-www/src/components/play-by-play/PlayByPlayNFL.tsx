...
import { Play } from "../../schema/nfl/playbyplay.ts";
...
export function PlayByPlayNFL({ gameId, onBack }: PlayByPlayNFLProps) {
  // Get gameId from URL params if not provided as prop
  const urlParams = new URLSearchParams(window.location.search);
  const gameIdFromUrl = urlParams.get("gameId");
  const actualGameId = gameId || gameIdFromUrl;

  // Get league from URL path (e.g., /scores/nfl/76782 -> nfl)
  const pathParts = window.location.pathname.split("/");
  const leagueFromUrl = pathParts[2]; // /scores/nfl/76782 -> nfl
  ...
  const { nflTeamProfiles, fetchNFLTeamProfiles } = useArb();
  ...
  useEffect(() => {
    ...
    if (currentLeague === League.NFL) {
      fetchNFLTeamProfiles();
    }
