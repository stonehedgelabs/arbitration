...
import { Play } from "../../schema/nba/playbyplay.ts";
...
export function PlayByPlayNBA({ gameId, onBack }: PlayByPlayNBAProps) {
  // Get gameId from URL params if not provided as prop
  const urlParams = new URLSearchParams(window.location.search);
  const gameIdFromUrl = urlParams.get("gameId");
  const actualGameId = gameId || gameIdFromUrl;

  // Get league from URL path (e.g., /scores/nba/76782 -> nba)
  const pathParts = window.location.pathname.split("/");
  const leagueFromUrl = pathParts[2]; // /scores/nba/76782 -> nba
  ...
  const { nbaTeamProfiles, fetchNBATeamProfiles } = useArb();
  ...
  useEffect(() => {
    ...
    if (currentLeague === League.NBA) {
      fetchNBATeamProfiles();
    }
            ...
            <Card.Body p="4">
              <VStack gap="3" align="stretch">
                ...
                <HStack justify="space-between" align="center">
                  ...
                  <HStack gap="2">
                    <QuarterBadge
                      quarterNumber={gameData.Quarter || 1}
                      league={League.NBA}
                      size="sm"
                    />
                    <Badge variant="outline" fontSize="xs">
                      {gameData.TimeRemainingMinutes || "0:00"}
                    </Badge>
                  </HStack>
                </HStack>
              ...
              <Card.Body p="4">
                <VStack gap="2" align="stretch">
                  <HStack justify="space-between" align="center">
                    <HStack gap="2">
                      <QuarterBadge
                        quarterNumber={play.Quarter || 1}
                        league={League.NBA}
                        size="sm"
                      />
                      <Badge variant="outline" fontSize="xs">
                        {play.TimeRemainingMinutes || "0:00"}
                      </Badge>
                      {index === 0 && (
