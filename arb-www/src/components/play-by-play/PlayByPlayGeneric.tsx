...
// Generic play interface for unsupported leagues
interface GenericPlay {
  PlayID: string;
  Description: string;
  Team: string;
  Quarter?: number;
...
interface PlayByPlayGenericProps {
  gameId?: string;
  league: League;
  onBack: () => void;
}
...
export function PlayByPlayGeneric({
  gameId,
  league,
  onBack,
}: PlayByPlayGenericProps) {
  // Get gameId from URL params if not provided as prop
  const urlParams = new URLSearchParams(window.location.search);
  const gameIdFromUrl = urlParams.get("gameId");
  const actualGameId = gameId || gameIdFromUrl;

  // Get league from URL path (e.g., /scores/golf/76782 -> golf)
  const pathParts = window.location.pathname.split("/");
  const leagueFromUrl = pathParts[2]; // /scores/golf/76782 -> golf
              ...
              <VStack gap="3" align="stretch">
                ...
                <HStack justify="space-between" align="center">
                  ...
                  <HStack gap="2">
                    ...
                    <Badge variant="outline" fontSize="xs">
                      {gameData.TimeRemainingMinutes || "TBD"}
                    </Badge>
                  </HStack>
                </HStack>
                ...
                <HStack gap="2" flexWrap="wrap">
                  <Badge variant="outline" fontSize="xs">
                    {gameData.Stadium || gameData.Arena || "TBD"}
                  </Badge>
                  <Badge variant="outline" fontSize="xs">
                    {gameData.City || "TBD"}
                  </Badge>
              ...
              <Card.Body p="4">
                <VStack gap="2" align="stretch">
                  <HStack justify="space-between" align="center">
                    <HStack gap="2">
                      <Badge variant="outline" fontSize="xs">
                        {play.Quarter || play.Period || "1"}
                      </Badge>
                      <Badge variant="outline" fontSize="xs">
                        {play.TimeRemainingMinutes || "TBD"}
                      </Badge>
                      {index === 0 && (
